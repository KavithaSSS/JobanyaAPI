    ...
    objUtilities.CheckValidUserOrEmployeeOrEmployer(req, function (validemp) {
      ...
      if (validemp == true) {
        ...
        if (Object.keys(req.body).length > 0) {
          ...
          objJobList.getAllJobList(logparams, empparams, listparams, 4, function (jobresult) {
                            ...
                            //return res.status(200).json({
                              ...
                              objUtilities.getcurrentmilliseconds(function (currenttime) {
                                ...
                                objJobApply.ApplyJobSave(logparams, saveparams, function (jobapply) {
                                  ...
                                  else if (jobapply.type == "Already Applied") {
                                    const msgparam = { "messagecode": varconstant.alreadyapplied };
                                    objUtilities.getMessageDetailWithkeys(msgparam, function (msgresult) {
                                      ////console.log("Hello");  
                                      ////console.log(prefresult);
                                      ////console.log("Hi");
                                      return res.status(200).json({
                                        job_json_result: {
                                          varstatuscode: varconstant.alreadyapplied,
                                          responsestring: msgresult[0].messagetext,
                                          responsekey: msgresult[0].messagekey,
                                          response: varconstant.successresponsecode,
                                          //job_list: jobresult
...
exports.updateCallCount = async function(req, res){
  try {
    ...
    objUtilities.CheckValidUserOrEmployeeOrEmployer(req, function (validemp) {
      if (validemp == true) {
        ...
        var params = { "ipaddress": req.query.deviceip, "usercode": logUserCode, "orginator": 'Call count for job', "type": logType };
        objUtilities.getLogDetails(params, function (logresponse) {
          objLogdetails = logresponse;
        });
        var logparams = objLogdetails;
        ...
        {
          // console.log("Entry2")
          empparams = { "employeecode": Number(req.query.employeecode), "jobcode": Number(req.query.jobcode) };
          objJobApply.AppliedCallCount(logparams, empparams, req, function(updateresult){
            // console.log("Entry3")
            if (updateresult != null && updateresult == true) {
              // console.log("Entry4")
        ...
        {
          empparams = { "employeecode": Number(req.query.employeecode), "jobcode": Number(req.query.jobcode) };
          objJobApply.InvitedCallCount(logparams, empparams, req, function(updateresult){
            // console.log("Entry3")
            if (updateresult != null && updateresult == true) {
              // console.log("Entry4")
              const msgparam = { "messagecode": varconstant.updatecode };
  ...
  catch (e) { logger.error("Error in update call count for Applied & Invited: " + e); }
}

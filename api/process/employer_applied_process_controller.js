...
exports.Appliedlist = function (logparams,req, callback) {
    try {
        logger.info("Applied List : UserId: " + logparams.usercode + ", Originator: " + logparams.orginator + ", DeviceIP: " + logparams.ipaddress + ", Logdate: " + logparams.logdate + ", Type: " + logparams.type);
        const dbo = MongoDB.getDB();
        var finalresult;
        dbo.collection(MongoDB.EmployeeAppliedCollectionName).aggregate([
            { $match: { "employercode": Number(req.query.employercode), "jobcode": Number(req.query.jobcode) } },
            {
                $lookup: {
        ...
        logger.error("Error in Applied List - Employer " + e);
    }
}

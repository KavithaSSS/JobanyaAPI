...
function checkSpecializationName(specializationListObj, educationcodeobj, callback) {
    try {
        ...
        const dbo = MongoDB.getDB();
        var temparray = [];
        // var params={"groupcode":Number(req.query.groupcode)};
...
exports.FindGroupcode = function (logparams, req, callback) {
    try {
        logger.info("Log in get Group Code: UserId: " + logparams.usercode + ", Originator: " + logparams.orginator + ", DeviceIP: " + logparams.ipaddress + ", Logdate: " + logparams.logdate);
        const dbo = MongoDB.getDB();
        dbo.collection(MongoDB.SpecializationCollectionName).find({ "groupcode": parseInt(req.query.groupcode) }, { projection: { _id: 0, specializationcode: 1 } }).toArray(function (err, result) {
            var specialization = [];
            var code = result;
        ...
        dbo.collection(MongoDB.SpecializationCollectionName).aggregate(
            [
                { $match: params },
                {
                    $group:
                    {
                        _id: { groupcode: "$groupcode", specialization: "$specialization", statuscode: '$statuscode' },
                        specializationcode: { $addToSet: "$specializationcode" },
                        educationcategorycode: { $addToSet: "$educationcategorycode" }
                    }
                },
                {
                    $project: { _id: 0, groupcode: "$_id.groupcode", specialization: "$_id.specialization", statuscode: '$_id.statuscode', specializationcode: '$specializationcode', educationcategorycode: '$educationcategorycode' }
                }
    ...
    try {
        ...
        dbo.collection(MongoDB.SpecializationCollectionName).aggregate([
            ...
            },
            {
                $group:
                {
                    _id: {specializationcode:'$specializationcode', specializationname: "$specialization.specializationname",languagecode: '$specialization.languagecode', groupcode: "$groupcode", statuscode: '$statuscode', statusname: "$status.statusname", language_count_status: "$langstatus", selected_language_count: "$selectedcount" },
                    educationcategoryname: { $addToSet: "$categorycode.educationcategory.educationcategoryname" }
                }
            },
            {
                $project: {
                    _id: 0, groupcode: "$_id.groupcode", statuscode: '$_id.statuscode', statusname: '$_id.statusname', educationcategoryname: '$educationcategoryname',
                    specializationname: '$_id.specializationname',specializationcode: '$_id.specializationcode',languagecode: '$_id.languagecode', language_count_status: "$_id.language_count_status", selected_language_count: "$_id.selected_language_count"
                }
            }]).toArray(function (err, result) {
                finalresult = result;
                ////console.log(result);
...
exports.Duplicatecheck = function (logparams, req, callback) {
    try {
        ...
        SpeclializationDuplicate(req.body.specialization, req.query.groupcode, function (err, specializationcount) {
            if (err) {
                return;
            }
            return callback(specializationcount);
...
function SpeclializationDuplicate(specializationListobj, groupobj, callback) {
    try {
        ...
        var iteratorFcn = function (specializationObj, done) {
            ...
            // var regex = new RegExp("^" + specializationObj.specializationname.toLowerCase(), "i");
            dbo.collection(MongoDB.SpecializationCollectionName).find({ specialization: { $elemMatch: { specializationname: { $regex: "^" + specializationObj.specializationname + "$", $options: 'i' }, languagecode: specializationObj.languagecode } }, groupcode: { $ne: Number(groupobj) } }).count(function (err, result) {
                // //console.log(result);
                if (err) {
                    done(err);
                    return;
...
function SingleRecordCheck(updateitem, req, callback) {
    try {
        ...
        var iteratorFcn = function (SingleObj, done) {
            ...
            dbo.collection(MongoDB.SpecializationCollectionName).updateOne({ "groupcode": Number(req.query.groupcode), "specializationcode": Number(SingleObj.specializationcode) }, { $set: SingleObj }, function (err, result) {
                if (err) {
                    done(err);
                    return;
                }
...
exports.CheckspecializationExitingMapping = function (logparams, req, filtered, callback) {
    try {
        ...
        Speclializationmapping(req.query.groupcode, filtered, function (err, totalcount) {
            if (err) {
                return;
            }
...
function Speclializationmapping(groupobj, filterListobj, callback) {
    try {
        ...
        var iteratorFcn = function (filterObj, done) {
            dbo.collection(MongoDB.SpecializationCollectionName).aggregate([
                { $match: { "groupcode": Number(groupobj), "educationcategorycode": filterObj } },
                {
                    $lookup: {

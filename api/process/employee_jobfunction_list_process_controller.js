...
exports.getJobFunctionCount = function (logparams, languagecode, employeecode, callback) {
  try {
    ...
    var jobcountconditions = { "jobcount": { "$gt": 0 } };
    if (employeecode == -1) {
      isshowwebsitecondition = { 'isshowwebsite': 1 }
      jobcountconditions = { $or: [{ "jobcount": { "$gt": 0 } }, { "flashjobcount": { "$gt": 0 } }] }
    }
    logger.info("Log in Job List Count by Job function Employee App : UserId: " + logparams.usercode + ", Originator: " + logparams.orginator + ", DeviceIP: " + logparams.ipaddress + ", Logdate: " + logparams.logdate + ", Type: " + logparams.type);
    ...
    objUtilities.FindAllDeletedEmployer(function (emplist) {
      ...
      if (emplist != null && emplist.length > 0) {
        for (var i = 0; i <= emplist.length - 1; i++) {
          tempempcode.push(emplist[i].employercode);
        }
        empcondition = { "jobslist.employercode": { $nin: tempempcode } };
      }
      //console.log(empcondition);
      dbo.collection(MongoDB.JobFunctionCollectionName).aggregate([
        ...
        {
          "$lookup": {
            "from": String(MongoDB.PrivateJobPostsCollectionName),
            "localField": "jobfunctioncode",
            "foreignField": "jobfunctioncode",
            "as": "flashjobslist"
          }
          ...
          "$addFields": {
            "jobslist": {
              "$filter": {
                ...
                "as": "filterjoblist",
                "cond": {
                  "$and": [{ "$gte": ["$$filterjoblist.validitydate", milliseconds] },
        ...
        { $match: { "jobfunction.languagecode": Number(languagecode) } },
        { $match: isshowwebsitecondition },
        {
          "$project": {
            ...
            "flashjobcount": { "$size": "$flashjobslist" },
          }
        },
        { $sort: { 'jobfunctionname': 1 } },
        { $match: jobcountconditions },
        // {
        //   "$match": {
        //     $or: [{ "jobcount": { "$gt": 0 } }, { "flashjobcount": { "$gt": 0 } }]
        //   }
        // }
  ...
  catch (e) { logger.error("Error in Getting Job List Count by Job function: " + e); }


}

exports.getJobListbyJobFunction = function (logparams, params, callback) {
  try {
    const dbo = MongoDB.getDB();
    var finalresult;
    var date = new Date(); // some mock date
    var milliseconds = date.getTime();
    logger.info("Log in Job List by Job Function Employee App : UserId: " + logparams.usercode + ", Originator: " + logparams.orginator + ", DeviceIP: " + logparams.ipaddress + ", Logdate: " + logparams.logdate + ", Type: " + logparams.type);
    //var dbcollectionname = dbo.collection(MongoDB.SplashCollectionName);
    //state Collection
    ////console.log(languagecode);

    dbo.collection(MongoDB.JobPostsCollectionName).aggregate([
      { $match: { $and: [{ 'validitydate': { $gte: milliseconds } }, { 'jobfunctioncode': { $eq: Number(params.typecode) } }, { 'statuscode': { $eq: objConstants.approvedstatus } }] } },
      {
        "$lookup": {
      ...
      { $match: { "districtinfo.district.languagecode": Number(params.languagecode) } },
      {
        $group: {
          "_id": {
            "jobid": "$jobid", "jobcode": '$jobcode', "jobrolecode": '$jobrolecode', "jobrolename": '$jobroleinfo.jobrole.jobrolename',
            "employercode": '$employercode', "employername": '$employerinfo.registeredname', "experience": '$experience', "salaryrange": '$salaryrange',
            "subscriptiondetails": '$subscriptiondetails', "noofopenings": '$noofopenings', "approveddate": '$approveddate', "validitydate": { $toDate: '$validitydate' }, "daysleft": {
              $ceil: {
                "$divide": [
  ...
  catch (e) { logger.error("Error in Getting Job List by Job Function: " + e); }

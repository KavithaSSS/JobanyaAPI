...
exports.getLocationCount = function (logparams, languagecode, employeecode, callback) {
  try {
    ...
    logger.info("Log in Job List Count by Location Employee App : UserId: " + logparams.usercode + ", Originator: " + logparams.orginator + ", DeviceIP: " + logparams.ipaddress + ", Logdate: " + logparams.logdate + ", Type: " + logparams.type);
      ...
      dbo.collection(MongoDB.DistrictCollectionName).aggregate([
...

        {
          "$lookup": {
            ...
            "foreignField": "preferredjoblocation.locationlist.locationcode",
            "as": "jobslist"
          }
        },
        {
        ...
        { $match: { "district.languagecode": Number(languagecode) } },
        { $match: isshowwebsitecondition },
        {
          "$project": {
            _id: 0,
            "districtcode": 1,
            "districtname": '$district.districtname', "statecode": 1,
            "imageurl": 1,
  ...
  catch (e) { logger.error("Error in Getting Job List Count by Location: " + e); }
}

exports.getEligibleLocationCount = function (logparams, languagecode, callback) {
  try {
    const dbo = MongoDB.getDB();
    var finalresult;
    var date = new Date(); // some mock date
...
exports.getJobListbyLocation = function (logparams, params, callback) {
  try {
    const dbo = MongoDB.getDB();
    var finalresult;
    var date = new Date(); // some mock date
    var milliseconds = date.getTime();
    logger.info("Log in Job List by Location Employee App : UserId: " + logparams.usercode + ", Originator: " + logparams.orginator + ", DeviceIP: " + logparams.ipaddress + ", Logdate: " + logparams.logdate + ", Type: " + logparams.type);
    //var dbcollectionname = dbo.collection(MongoDB.SplashCollectionName);
    //state Collection
    ////console.log(languagecode);
    dbo.collection(MongoDB.JobPostsCollectionName).aggregate([
      { $match: { $and: [{ 'validitydate': { $gte: milliseconds } }, { 'preferredjoblocation.locationlist.locationcode': { $eq: Number(params.typecode) } }, { 'statuscode': { $eq: objConstants.approvedstatus } }] } },
      {
        "$lookup": {
  ...
  catch (e) { logger.error("Error in Getting Job List by Location: " + e); }
}
...
exports.getJobTypeCount = function (logparams, languagecode, callback) {
    try {
        const dbo = MongoDB.getDB();
        var finalresult;
        var date = new Date(); // some mock date
        var milliseconds = date.getTime();
        logger.info("Log in Job List Count by Job Type Employee App : UserId: " + logparams.usercode + ", Originator: " + logparams.orginator + ", DeviceIP: " + logparams.ipaddress + ", Logdate: " + logparams.logdate + ", Type: " + logparams.type);
                ...
                dbo.collection(MongoDB.JobTypeCollectionName).aggregate([
                  {$unwind:"$jobtype"},
                   ...
                   { "$lookup": {
                    ...
                    {$match:{"jobtype.languagecode":Number(languagecode)} },
                    { "$project": {
                    _id: 0,
                      "jobtypecode": 1,
    ...
    catch (e) { logger.error("Error in Getting Job List Count by Job Type: " + e); }


}

exports.getJobTypeList = function (logparams, params, callback) {
  try {
      const dbo = MongoDB.getDB();
      var finalresult;
      var date = new Date(); // some mock date
      var milliseconds = date.getTime();
      logger.info("Log in Job List by Job Type Employee App : UserId: " + logparams.usercode + ", Originator: " + logparams.orginator + ", DeviceIP: " + logparams.ipaddress + ", Logdate: " + logparams.logdate + ", Type: " + logparams.type);
      //var dbcollectionname = dbo.collection(MongoDB.SplashCollectionName);
      //state Collection
      ////console.log(languagecode);
      dbo.collection(MongoDB.JobPostsCollectionName).aggregate([
        {$match:{  $and: [{'validitydate': {$gte: milliseconds }},{'jobtypecode': {$eq: Number(params.typecode)}},{'statuscode': {$eq: objConstants.approvedstatus }}]}},
         { "$lookup": {
  ...
  catch (e) { logger.error("Error in Getting Job List by Job Type: " + e); }
